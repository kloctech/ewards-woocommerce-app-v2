...
export default class SyncCustomersService {
  ...
  async execute(){
    let response = await this.WooCommerce.get("customers?page=1&per_page=100")
    .catch((error) => {
  ...
  async #createCustomers(customers){
    let customerData = await this.#processCustomersData(customers)
    let insertCustomer = await Customer.insertMany(customerData)
    .catch(function(err) {
      console.log(err);
  ...
  #processCustomersData(customers){
    ...
    customers.map((customer) => {
      customersData.push({
        ...
        address: `${customer.billing.address_1}-${customer.billing.address_1}`,
